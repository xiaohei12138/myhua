...
void add_my_log(const char *fmt, ...)
{
	...
	va_end(args);
	buffer = kzalloc(strlen(tmp_buff)+1,GFP_KERNEL);
	strcpy(buffer,tmp_buff);
	//strncpy(buffer,tmp_buff,sizeof(buffer));
	my_buff[num] = buffer;
	num++;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for(i=0;i<num;i++)
	{
		if(strlen(my_buff[i])>0)
			ret+=sprintf(buf,"%s%s",buf,my_buff[i]);
	}
	return ret;
}
static DEVICE_ATTR(my_log,0664, my_log_show, NULL);
